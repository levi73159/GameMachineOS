...
int FAT_CompareLFNBlock(const void* blockA, const void* blockB)
{
...
void FAT_GetRootDirectory(uint32_t *rootDirLba, uint32_t *rootDirSize, bool isFat32)
{
    if (isFat32)
    {
...
bool FAT_Initialize(Partition *disk)
{
    ...
    bool isFat32 = FAT_DetectFat32();
    // open root directory file
    uint32_t rootDirLba;
    uint32_t rootDirSize;
    FAT_GetRootDirectory(&rootDirLba, &rootDirSize, isFat32);
...
FAT_FileData *FAT_GetFileData(FAT_DirectoryEntry *entry, int handle)
{
...
FAT_File *FAT_OpenEntry(Partition *disk, FAT_DirectoryEntry *entry)
{
    ...
    // setup vars
    FAT_FileData *fd = FAT_GetFileData(entry, handle);
...
uint32_t FAT_GetFatIndex(uint32_t currentCluster)
{
    if (g_FatType == 12)
    {
        return currentCluster * 3 / 2;
    }
    else if (g_FatType == 16)
    {
        return currentCluster * 2;
    }
    else /*if (g_FatType == 32)*/
    {
        return currentCluster * 4;
    }
}

uint32_t FAT_CalculateNextCluster(uint32_t currentCluster, uint32_t fatIndex)
{
...
uint32_t FAT_NextCluster(Partition *disk, uint32_t currentCluster)
{
    ...
    uint32_t fatIndex = FAT_GetFatIndex(currentCluster);
    ...
    return FAT_CalculateNextCluster(currentCluster, fatIndex);
}

bool FAT_ReadNextSector(Partition *disk, FAT_FileData *fd)
{
...
bool FAT_FindFile(Partition *disk, FAT_File *file, const char *name, FAT_DirectoryEntry *entryOut)
{
    ...
    while (FAT_ReadEntry(disk, file, &entry))
    {
        ...
        //     // if this the last LFN Block
        //     if ((lfn->Order & FAT_LFN_LAST) != 0)
        //     {
        //         qsort(g_Data->LFNBlocks, g_Data->LFNCount, sizeof(FAT_LFNBlock), FAT_CompareLFNBlock);
