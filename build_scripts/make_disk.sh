...
mkfs.fat -n "GMOS" $TARGET_PARTITION >/dev/null
...
# copy files
echo "Copying files to ${TARGET_PARTITION} (mounted on /tmp/gmos)..."
mkdir -p /tmp/gmos
mount ${TARGET_PARTITION} /tmp/gmos
cp ${BUILD_DIR}/kernel.bin /tmp/gmos
cp test.txt /tmp/gmos
mkdir /tmp/gmos/test
cp test.txt /tmp/gmos/test
umount /tmp/gmos
